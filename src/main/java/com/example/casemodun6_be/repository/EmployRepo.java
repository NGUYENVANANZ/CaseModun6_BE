...
public interface EmployRepo extends CrudRepository<Employ, Long> {
    @Query(nativeQuery = true,value = "SELECT employ.id, employ.account_id, employ.detail_account_id, sum(employ.hires) as hires FROM employ GROUP BY detail_account_id ORDER BY hires DESC limit 12")
